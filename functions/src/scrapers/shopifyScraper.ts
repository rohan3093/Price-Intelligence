...
import { MarketplaceScraper, ScrapedListing, MarketChannel } from "./types";
...
interface ShopifyOption {
  id: number;
  product_id: number;
  name: string;      // e.g. "Size", "Color", "Title"
...
interface ShopifyVariant {
  id: number;
  title: string;        // e.g. "BLACK / UK 9" or "UK 9"
  price: string;        // Price as string, e.g. "26500.00"
  available: boolean | null; // null for consignment/marketplace items
  sku: string | null;
  option1?: string | null;
  option2?: string | null;
  option3?: string | null;
...
export interface ShopifyStoreConfig {
  /** Unique ID matching your IndianMarketplace type */
  id: string;
  /** Human-readable name */
  displayName: string;
  /** Base URL without trailing slash, e.g. "https://www.crepdogcrew.com" */
  baseUrl: string;
  /** Channel category */
  ...
  enabled: boolean;
  /**
   * Optional function to normalise a variant title into a standard
   * size string like "UK 9".  If not provided, the default normaliser
   * is used (handles common patterns like "9", "UK9", "US 10", etc.).
   */
  normaliseSize?: (variantTitle: string) => string;
  /**
   * Optional function to check if a Shopify product matches the asset
   * we're looking for. Defaults to fuzzy name matching.
   */
  matchesAsset?: (product: ShopifyProduct, assetName: string, sku: string) => boolean;
  /**
 ...
 */
function defaultNormaliseSize(raw: string): string {
...
function extractSizeFromVariant(
  ...
  if (variant.title) {
    const sizeMatch = variant.title.match(
      /\b(UK\s?\d[\d.]*|US\s?\d[\d.]*|EU\s?\d[\d.]*|\d+\.?\d*\s*UK|\d+\.?\d*\s*US|\d+\.?\d*\s*EU|\d+\.?\d*)\b/i
    );
    if (sizeMatch) {
      return sizeMatch[1];
    }
  ...
  console.warn(
    `[Shopify] Could not extract size from variant ${variant.id} (title: "${variant.title}", ` +
 ...
 */
function assetNameToSearchQuery(assetName: string): string {
...
function defaultMatchesAsset(
  ...
  if (normalProduct === normalAsset) return true;
  ...
  if (sku) {
    ...
    if (
      product.variants.some(
        (v) =>
          v.sku && v.sku.toLowerCase().replace(/[-\s]/g, "") === skuClean
      )
    ) {
      return true;
    }
    ...
    const tagsArr = getTagsArray(product.tags as any);
    if (
      tagsArr.some(
        (tag) => tag.toLowerCase().replace(/[-\s]/g, "") === skuClean
      )
    ) {
      return true;
    }
...
async function fetchProducts(
  ...
  for (const searchQuery of searchQueries) {
    ...
    try {
      const searchUrl =
        `${config.baseUrl}/search/suggest.json?q=${encodeURIComponent(searchQuery)}` +
      ...
      if (searchRes.ok) {
        const searchData: ShopifySearchSuggestResponse = await searchRes.json();
        const suggestedProducts =
          searchData?.resources?.results?.products || [];

        console.log(
          `[${config.id}] Search returned ${suggestedProducts.length} suggestion(s) for "${searchQuery}"`
        );
      ...
      console.warn(
        `[${config.id}] Search suggest failed for "${searchQuery}":`,
        err
      );
    }
...
function shopifyProductToListings(
  product: ShopifyProduct,
  sku: string,
  config: ShopifyStoreConfig
): ScrapedListing[] {
  ...
  console.log(
    `[${config.id}] "${product.title}": ${totalVariants} variants → ` +
...
export function createShopifyScraper(
  config: ShopifyStoreConfig
): MarketplaceScraper {
  return {
    id: config.id,
    ...
    async scrapeBysku(sku: string, assetName?: string): Promise<ScrapedListing[]> {
      const searchName = assetName || sku;
      console.log(
        `[${config.id}] Searching for "${searchName}" (SKU: ${sku}) on ${config.displayName}...`
      );
    ...
    async scrapeByQuery(queryStr: string): Promise<ScrapedListing[]> {
      console.log(
        `[${config.id}] Searching for "${queryStr}" on ${config.displayName}...`
      );
...
export async function scrapeShopifyStore(
  ...
  console.log(
    `[${config.id}] Searching for "${assetName}" (${sku}) on ${config.displayName}...`
  );
...
function processProducts(
  ...
  console.log(
    `[${config.id}] Found ${products.length} matching product(s)`
  );
  ...
  console.log(
    `[${config.id}] ${deduped.length} listings (${allListings.length} before dedup)`
  );
...
async function fetchFullCatalog(
  ...
  console.log(
    `[${config.id}] Catalog loaded: ${allProducts.length} products in ${page} page(s)`
  );
  return allProducts;
}
 ...
 */
export async function scrapeStoreBulk(
  config: ShopifyStoreConfig,
  assets: Array<{ id: string; sku: string; name: string }>
): Promise<Map<string, ScrapedListing[]>> {
  const resultMap = new Map<string, ScrapedListing[]>();
  const matchesAsset = config.matchesAsset || defaultMatchesAsset;
  ...
  for (const asset of assets) {
    if (!asset.sku && !asset.name) continue;

    const matchingProducts = catalog.filter((product) =>
      matchesAsset(product, asset.name, asset.sku)
    );
  ...
  console.log(
    `[${config.id}] Bulk scrape complete: matched ${resultMap.size}/${assets.length} assets, ` +
 ...
 */
export function extractProductJsonUrl(fullUrl: string): string | null {
  try {
...
): Promise<{ product: ShopifyProduct | null; jsonUrl: string }> {
  const jsonUrl = extractProductJsonUrl(productPageUrl);
  if (!jsonUrl) {
    console.warn(
      `[fetchProductByUrl] Could not extract JSON URL from: ${productPageUrl}`
    );
 ...
 * analyst-entered URL).
 */
export function convertProductToListings(
  product: ShopifyProduct,
  sku: string,
  storeId: string,
  storeName: string,
