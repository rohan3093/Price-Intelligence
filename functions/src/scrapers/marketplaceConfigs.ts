 ...
 */
export function getEnabledShopifyConfigs(): ShopifyStoreConfig[] {
  return SHOPIFY_MARKETPLACE_CONFIGS.filter((c) => c.enabled);
}

/**
 * Get a specific Shopify config by marketplace ID.
 */
export function getShopifyConfigById(id: string): ShopifyStoreConfig | undefined {
  return SHOPIFY_MARKETPLACE_CONFIGS.find((c) => c.id === id);
}

export const SHOPIFY_MARKETPLACE_CONFIGS: ShopifyStoreConfig[] = [
