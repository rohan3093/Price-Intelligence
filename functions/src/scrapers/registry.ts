...
import { MarketplaceScraper } from "./types";
import { createShopifyScraper, ShopifyStoreConfig } from "./shopifyScraper";
import { SHOPIFY_MARKETPLACE_CONFIGS } from "./marketplaceConfigs";
import {
  discoverAndVerifyShopifyStores,
  DiscoveryResult,
} from "./urlDiscovery";

// ── Static scrapers (always available, no async) ─────────────────────

const staticShopifyScrapers: MarketplaceScraper[] =
  SHOPIFY_MARKETPLACE_CONFIGS.filter((c) => c.enabled).map(createShopifyScraper);

// Custom (non-Shopify) scrapers can be added here
const customScrapers: MarketplaceScraper[] = [];

// ── Simple getters for when you don't need discovery ─────────────────

/** Get statically configured + enabled scrapers (no Firestore scan) */
export function getEnabledScrapers(): MarketplaceScraper[] {
  return [...staticShopifyScrapers, ...customScrapers];
}

/** Get a specific scraper by marketplace ID (static only) */
export function getScraper(id: string): MarketplaceScraper | undefined {
  return [...staticShopifyScrapers, ...customScrapers].find(
    (s) => s.id === id
  );
}
 ...
 */
export async function buildScrapers(): Promise<{
  scrapers: MarketplaceScraper[];
  discovery: DiscoveryResult;
}> {
  // 1. Discover from Firestore
  const { configs: discoveredConfigs, discovery } =
    await discoverAndVerifyShopifyStores();

  // 2. Build a set of IDs from static configs (all of them, even disabled)
  const staticIds = new Set(
    SHOPIFY_MARKETPLACE_CONFIGS.map((c) => c.id)
  );

  // 3. Find discovered stores that aren't in the static list
  const newDiscoveredConfigs = discoveredConfigs.filter(
    (dc) => !staticIds.has(dc.id)
  );
  ...
  const discoveredUrlMap = new Map(
    discoveredConfigs.map((dc) => [dc.id, dc.baseUrl])
  );

  const mergedStaticConfigs: ShopifyStoreConfig[] =
    SHOPIFY_MARKETPLACE_CONFIGS.map((staticConfig) => {
      ...
      if (discoveredUrl && !staticConfig.enabled) {
        ...
        console.log(
          `[Registry] Enabling "${staticConfig.id}" — URL discovered from analyst data: ${discoveredUrl}`
        );
        return {
          ...staticConfig,
      ...
      if (discoveredUrl && discoveredUrl !== staticConfig.baseUrl) {
        ...
        console.log(
          `[Registry] Updating "${staticConfig.id}" URL: ${staticConfig.baseUrl} → ${discoveredUrl}`
        );
        return {
          ...staticConfig,
          baseUrl: discoveredUrl,
  ...
  const allConfigs: ShopifyStoreConfig[] = [
    ...mergedStaticConfigs.filter((c) => c.enabled),
    ...newDiscoveredConfigs,
  ];

  const scrapers: MarketplaceScraper[] = [
    ...allConfigs.map(createShopifyScraper),
    ...customScrapers,
  ];

  console.log(
    `[Registry] Built ${scrapers.length} scraper(s): ` +
...
export function getScraperStatus(): Array<{
  ...
  return SHOPIFY_MARKETPLACE_CONFIGS.map((s) => ({
    id: s.id,
