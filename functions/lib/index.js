...
const parseReleaseDateTimeIST = (drop) => {
    if (!(drop === null || drop === void 0 ? void 0 : drop.releaseDate))
        return null;
...
const canSendEmail = (email, emailEnabled) => {
 ...
 */
exports.sendDropReminders = functions
    .region("asia-south1")
        ...
        for (const userDoc of usersSnapshot.docs) {
            ...
            for (const reminder of reminders) {
                ...
                if (reminderTime >= now && reminderTime <= oneMinuteFromNow) {
                    ...
                    const sendPromises = fcmTokens.map((token) => admin.messaging().send(Object.assign(Object.assign({}, message), { token: token })).catch((error) => {
 ...
 */
exports.scrapeMarketplacePrices = functions
    .region("asia-south1")
    .runWith({
    ...
    try {
        ...
        const result = await (0, orchestrator_1.runDailyScrape)(undefined, true);
        console.log(`Scheduled scrape complete: ${result.totalListings} listings, ` +
 ...
 */
exports.manualScrapeMarketplaces = functions
    .region("asia-south1")
    .runWith({
    timeoutSeconds: 540,
    memory: "512MB",
 ...
 */
exports.scrapeAssetPrices = functions
    .region("asia-south1")
    .runWith({
    timeoutSeconds: 120,
    memory: "256MB",
    ...
    try {
        const result = await (0, orchestrator_1.scrapeAsset)(assetId);
        res.json({
            success: true,
            assetId,
    ...
    try {
        ...
        const assetsSummary = assetsSnap.docs.map((d) => {
            ...
            return {
                ...
                sizesCount: ((_a = data.sizes) === null || _a === void 0 ? void 0 : _a.length) || 0,
            };
        });
        res.json({
            success: true,
    ...
    try {
        ...
        if (shouldVerify) {
            for (const marketplace of discovery.discovered) {
                try {
                    marketplace.isShopify = await (0, urlDiscovery_1.isShopifyStore)(marketplace.baseUrl);
                }
                catch (_a) {
        ...
        const knownStores = await (0, orchestrator_1.getScraperStatus)();
        const knownIds = new Set(knownStores.map((s) => s.id));
        const comparison = discovery.discovered.map((d) => (Object.assign(Object.assign({}, d), { inCurrentData: knownIds.has(d.id) })));
        res.json({
            success: true,
            ...
            knownStores,
            note: "Marketplaces are auto-discovered from analyst-entered listing URLs. " +
