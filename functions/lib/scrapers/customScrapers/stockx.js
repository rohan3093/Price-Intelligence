...
exports.extractStockXSlug = extractStockXSlug;
exports.stockxUkSize = stockxUkSize;
 ...
 */
function extractStockXSlug(productUrl) {
    try {
 ...
 */
function stockxUkSize(displayOptions, baseSize) {
    ...
    const usM = displayOptions.find((o) => o.type === "us m" || o.type === "us" || o.type === "us men");
    if (usM) {
...
async function fetchStockXProduct(_productUrl, _assetSku) {
    if (!_disabledNoticeLogged) {
        _disabledNoticeLogged = true;
        console.log("[StockX] scraper disabled — awaiting StockX Developer API " +
