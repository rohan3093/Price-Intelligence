...
exports.extractGoatSlug = extractGoatSlug;
exports.goatUsToUkSize = goatUsToUkSize;
 ...
 */
function extractGoatSlug(productUrl) {
    try {
 ...
 */
function goatUsToUkSize(usSize, gender) {
...
async function fetchGoatProduct(_productUrl, _assetSku) {
    if (!_disabledNoticeLogged) {
        _disabledNoticeLogged = true;
        console.log("[GOAT] scraper disabled — GOAT's /api/v1/* endpoints are behind " +
