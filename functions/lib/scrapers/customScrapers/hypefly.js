...
exports.fetchHypeflyProduct = fetchHypeflyProduct;
...
function parseNextData(html) {
    ...
    try {
        ...
        const product = ((_b = (_a = data === null || data === void 0 ? void 0 : data.props) === null || _a === void 0 ? void 0 : _a.pageProps) === null || _b === void 0 ? void 0 : _b.product) ||
            ((_e = (_d = (_c = data === null || data === void 0 ? void 0 : data.props) === null || _c === void 0 ? void 0 : _c.pageProps) === null || _d === void 0 ? void 0 : _d.data) === null || _e === void 0 ? void 0 : _e.product);
...
// ── Main fetch function ───────────────────────────────────────────────
async function fetchHypeflyProduct(productUrl, assetSku) {
    let fetchUrl;
    try {
    ...
    try {
        ...
        console.warn(`[HypeFly] Failed to fetch ${fetchUrl}: ${err.message}`);
        return [];
    }
}
function convertStrapiToListings(product, assetSku) {
    var _a;
    const imageData = (_a = product.images) === null || _a === void 0 ? void 0 : _a.data;
    const image = imageData && imageData.length > 0
    ...
    for (const variant of product.productVariants) {
        ...
        if (!rawSize) {
            skippedNoSize++;
            console.warn(`[HypeFly] Variant ${variant.id} has no size field`);
            continue;
        }
    ...
    console.log(`[HypeFly] "${product.name}" (__NEXT_DATA__): ${listings.length} listing(s) from ${product.productVariants.length} variants ` +
        `(${inStockCount} in-stock, skipped: ${skippedNoPrice} no-price, ${skippedNoSize} no-size)`);
    return listings;
}
function convertJsonLdToListings(product, assetSku, pageUrl) {
    ...
    for (const offer of offers) {
        ...
        if (!rawSize) {
            console.warn(`[HypeFly] Could not extract size from offer: ` +
    ...
    console.log(`[HypeFly] "${product.name}" (JSON-LD): ${listings.length} listing(s) from ${offers.length} offers ` +
 ...
 */
function extractSizeFromOffer(offer) {
    ...
    for (const field of [offer.name, offer.sku]) {
        if (!field)
            continue;
        const match = field.match(/\b(UK\s?\d[\d.]*|US\s?\d[\d.]*|EU\s?\d[\d.]*)\b/i);
        if (match)
            return match[1];
    }
    ...
    if (offer.url) {
        try {
            ...
            if (sizeParam && looksLikeSize(sizeParam))
                return sizeParam;
        }
        catch (_a) {
            // Invalid URL — skip
...
//# sourceMappingURL=hypefly.js.map