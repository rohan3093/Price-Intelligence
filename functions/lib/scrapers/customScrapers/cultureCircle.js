...
exports.fetchCultureCircleProduct = fetchCultureCircleProduct;
 ...
 */
async function fetchCultureCircleProduct(productUrl, assetSku) {
    var _a, _b;
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(`[CultureCircle] ${fetchUrl} returned ${res.status}`);
            return [];
        }
        ...
        if (!product) {
            console.warn(`[CultureCircle] No Product JSON-LD found on ${fetchUrl}`);
            return [];
        }
        // Extract listings from offers
        const offers = ((_a = product.offers) === null || _a === void 0 ? void 0 : _a.offers) || [];
        if (offers.length === 0) {
            console.warn(`[CultureCircle] Product "${product.name}" has no size offers`);
            return [];
        }
        const image = ((_b = product.image) === null || _b === void 0 ? void 0 : _b[0]) || undefined;
        const listings = [];
        ...
        for (const offer of offers) {
            ...
            if (!size) {
                skippedNoSize++;
                console.warn(`[CultureCircle] Could not normalise size from offer name: "${offer.name}"`);
                continue;
            }
            listings.push({
                sku: assetSku,
        ...
        console.log(`[CultureCircle] "${product.name}": ${listings.length} listing(s) from ${offers.length} offers ` +
            `(${inStockCount} in-stock, skipped: ${skippedNoPrice} no-price, ${skippedNoSize} no-size)`);
        return listings;
    }
    catch (err) {
        console.warn(`[CultureCircle] Failed to fetch ${fetchUrl}: ${err.message}`);
        return [];
    }
}
//# sourceMappingURL=cultureCircle.js.map