      ...
      <main className="flex-1 min-h-0 bg-brand-background px-2 py-2 md:px-3 md:py-3 pb-20 md:pb-4 w-full max-w-8xl mx-auto overflow-y-auto">
        <div className="flex items-center gap-2 mb-3">
          <button
            ...
            className="px-3 py-2 text-xs font-medium text-brand-black border border-brand-gray/30 bg-terminal-surface hover:border-terminal-border-strong flex items-center gap-1.5 transition leading-tight"
          >
            ← Back to Guides
          </button>
        </div>
        <article
          className="border border-brand-gray/20 p-4 md:p-6 bg-terminal-surface"
        >
          ...
          <div className="prose prose-sm max-w-none text-xs font-body text-brand-black leading-snug">
            <ReactMarkdown
              ...
              components={{
                ...
                blockquote: ({ children }) => (
                  <blockquote className="border-l-4 border-terminal-border-strong pl-3 py-1.5 my-2.5 bg-brand-gray/5 text-xs text-brand-black leading-snug [&>p]:mb-0">
                    {children}
                  </blockquote>
                ),
                table: ({ children }) => (
                  <div className="my-3 overflow-x-auto border border-brand-gray/20 rounded-lg bg-terminal-surface">
                    <table className="w-full text-[11px] leading-snug">{children}</table>
                  </div>
                ),
                thead: ({ children }) => (
                  <thead className="bg-terminal-surface-raised border-b border-brand-gray/20">{children}</thead>
                ),
                th: ({ children }) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {filteredGuides.map((guide) => (
          <button
            ...
            className="border border-brand-gray/20 p-4 bg-terminal-surface hover:border-terminal-border-strong text-left transition-all"
          >
